...
def modelInference(model_list):
    ...
    for file in tqdm(os.listdir(dataset)[:300]):
        #image = Image.open(os.path.join(dataset, file), 'r')
        data[len(imgind),:] = prepare(os.path.join(dataset, file),batchshape[1]) # Preprocessing
        imgind.append(int(file[-13:-5])-1) #indices of the images in the val.txt
    ...
    if (len(imgind) != batch and len(imgind) != 0):
        start = process_time()    
        prediction = model.predict(data[:len(imgind)],batch_size = batch)
        latency.append(process_time()-start)
        avg_lat.append(latency[-1]/(len(imgind)))
    ...
    t1_acc = round((t1/(300))*100,1) #top-1 accuracy
    t5_acc = round((t5/(300))*100,1) #top-5 accuracy
    throughput = round((300)/sum(latency),2) 
    tracemalloc.stop()
